...
export async function listPublicSessions(db: Queryable, accountId: string): Promise<SessionSummary[]> {
...
export async function readOwnSession(db: Queryable, accountId: string, sessionId: string): Promise<SessionSummary | null> {
  const result = await db.query<SessionRow>(
