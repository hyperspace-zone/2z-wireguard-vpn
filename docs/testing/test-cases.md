...
| PERF-002 | Gate RTT/jitter/loss comparison | Inspect the `Gate benchmark routes — RTT` table or API response. | Each completed row shows DoubleZero RTT p50, Internet RTT p50, RTT improvement, RTT saved, DoubleZero RTT jitter, Internet RTT jitter, RTT jitter improvement, RTT jitter saved, DoubleZero loss, Internet loss, loss improvement, loss saved, ingress gate ↔ DZ RTT, and egress gate ↔ DZ RTT. The ingress/egress columns come from each gate heartbeat's local DoubleZero edge RTT. Positive improvement means DoubleZero is faster or loses fewer packets. | Gate benchmark route table |
