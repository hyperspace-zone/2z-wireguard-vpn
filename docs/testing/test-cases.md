...
| PERF-002 | Gate RTT/jitter comparison | Inspect the `Gate benchmark routes — RTT` table or API response. | Each completed row shows DoubleZero RTT p50, Internet RTT p50, RTT improvement, RTT saved, ingress gate -> DZ RTT, egress gate -> DZ RTT, DoubleZero RTT jitter, Internet RTT jitter, RTT jitter improvement, and RTT jitter saved. Positive improvement means DoubleZero is faster. | Gate benchmark route table |
