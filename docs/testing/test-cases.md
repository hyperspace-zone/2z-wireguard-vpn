...
| OBS-006 | Gate disk janitor metrics | Run `systemctl start hyperspace-disk-janitor.service` on a gate, then scrape node exporter. | `hyperspace_gate_disk_janitor_last_run_timestamp_seconds`, before/after disk gauges, last action, and runs counter are present through the `hyperspace-gate-node` job. | Gate live smoke/manual |
