...
| OBS-015 | Drained DoubleZero device recovery and critical alert | Report an enabled gate on a device whose live status remains `drained` for two minutes. | The agent runs one guarded disconnect/connect cycle, verifies BGP plus routes, persists the result, and observes a six-hour cooldown. The alert is critical and states whether recovery failed or manual action is required. Healthy, non-drained, and transient states never trigger disconnect. | Gate-agent unit tests, Prometheus rule tests, managed canary rollout |
