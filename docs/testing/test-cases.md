...
| OBS-004 | Prometheus alert rules | Run `promtool check config`, `promtool check rules`, and `promtool test rules`, then inspect `/prometheus/alerts`. | Rules load without errors and cover service-host down/disk/inode/RAM/OOM/CPU, PostgreSQL exporter/connections/transactions/autovacuum/growth/WAL/backup age, HTTP/TCP blackbox probes, TLS expiry, gate resources, business state, Helius quota/usage health, Solana history scan freshness, and benchmark routes. | `infra/observability/prometheus/rules/hyperspace-alerts.yml` |
...
| OBS-015 | DoubleZero device/BGP recovery and critical alert | Report an enabled gate on a device whose live status remains `drained`, or whose assigned tunnel remains exactly `BGP Session Failed`, for two minutes. | The agent runs one guarded disconnect/connect cycle, verifies BGP plus routes, persists the result, and observes a six-hour cooldown. The alert is critical and states whether recovery failed or manual action is required. Healthy, pending, unidentified-device, transient, and unlisted states never trigger disconnect. | Gate-agent unit tests, Prometheus rule tests, managed canary rollout |
| OBS-016 | Benchmark failure isolation | Block a gate benchmark executor while a control job is queued, then load a non-benchmark web view while the matrix endpoint is unavailable. | The control lane claims and reports the assignment without waiting for the probe; heartbeat remains fresh; dashboard and config creation do not request or wait for the benchmark matrix. | `apps/gate-agent/cmd/hyperspace-gate-agent/main_test.go`, `packages/control-plane/src/resources/jobs/repository.test.ts`, staging live canary |
| OBS-017 | WireGuard assignment restoration | Persist one active committed assignment and one revoked assignment, simulate absent kernel state, and restart/test the gate agent. Then simulate a restoration error during a managed rollout. | The active assignment is recommitted from its persisted plan, the revoked record is skipped, startup reports the exact result, and a failure is critical and causes immediate managed rollback. | Gate-agent and deployment-controller unit tests, Prometheus rule test, managed canary rollout |
