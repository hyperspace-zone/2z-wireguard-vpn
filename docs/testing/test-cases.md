...
| PERF-002 | Gate RTT/jitter/loss comparison | Inspect the `Gate benchmark routes — RTT` table or API response. | Each completed row shows DoubleZero RTT p50, Internet RTT p50, RTT improvement, DoubleZero RTT jitter, Internet RTT jitter, RTT jitter improvement, RTT jitter saved, and loss. Positive improvement means DoubleZero is faster. | Gate benchmark route table |
